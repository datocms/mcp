...
/**
 * Extracts all method names from a resource class
 *
...
export function extractAllMethodNames(
	...
	// Find the resource property on the Client class
	...
	// Get all properties (methods) from the resource type
	const methodNames: string[] = [];
	const properties = resourceType.getProperties();

	for (const prop of properties) {
		const propType = checker.getTypeOfSymbol(prop);
		// Check if it's a method (has call signatures)
