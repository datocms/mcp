...
function createElement(type: string, props?: Record<string, any>, ...children: MarkdownNode[]): MarkdownElement {
  return {
    type,
    props: props || {},
...
function renderNode(node: MarkdownNode, depth: number = 0): string {
  ...
  const childrenText = children.map(child => renderNode(child, depth + 1)).join('');
  ...
  switch (type) {
    ...
    case 'ol':
      return children.map((child, index) => {
        if (typeof child === 'object' && child.type === 'li') {
...
function renderListItem(node: MarkdownElement, depth: number, number?: number): string {
  const indent = '  '.repeat(depth);
  const bullet = number ? `${number}.` : '-';
  const childrenText = node.children.map(child => renderNode(child, depth + 1)).join('');
  return `${indent}${bullet} ${childrenText}\n`;
}

// Main render function
export function render(...nodes: MarkdownNode[]): string {
  return nodes.map(node => renderNode(node)).join('').trim();
}
