...
// Extract client configuration and error types with their dependencies
const clientTypes = extractTypeDependencies(checker, program, [
	"ClientConfigOptions",
]);
	...
	simplifiedRegisterTool(
		...
		"create_script",
		{
			...
			description: render(
				...
				blockquote("⚠️ If any checkbox is unchecked, CODE WRITING IS FORBIDDEN"),
				...
				h1("CLIENT GENERAL RULES"),
				p(
					"The ",
					code("buildClient()"),
					" function supports the following configuration object:",
				),
				pre({ language: "typescript" }, clientTypes),
